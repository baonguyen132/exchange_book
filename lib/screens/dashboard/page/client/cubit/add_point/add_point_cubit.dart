...
class AddPointCubit extends Cubit<AddPointState> {
  AddPointCubit(UserModel userModel) : super(AddPointState.initial(
    path: "",
    idPurchasePoint: "",
    amout: 0,
    errorAmout: "",
  ...
  void loading() async {
    ...
    if (newPath.isNotEmpty && state.path != newPath) {
      ...
      emit(state.copyWith(path: newPath , idPurchasePoint: state.user.cccd+generate8DigitNumber().toString() , amout: 0));
    }
  ...
  void exchangePoint(int value)  {
    if(value < 10000){
      emit(state.copyWith(amout: value , errorAmout: "Giá trị phải hơn hoặc bằng 10.000"));
    }
    else {
